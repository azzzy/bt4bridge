...
- **Full Button Support**: All 4 buttons mapped to MIDI CC 80-83 on channel 0 (**fixed, hardware-assigned**)
...
- **Keyboard Mode**: Map buttons to keyboard keys for controlling any application (menu bar app only)
...
- **Keyboard Mode**: Map PG_BT4 buttons to any keyboard key or shortcut
- **Customizable Key Mappings**: GUI for remapping buttons to different keys
- **Persistent Settings**: Output mode and key mappings saved between sessions
...
5. Press the foot switches - they will send MIDI CC messages:
   - Button 1: CC 80 (**fixed**)
   - Button 2: CC 81 (**fixed**)
   - Button 3: CC 82 (**fixed**)
   - Button 4: CC 83 (**fixed**)
   - All buttons use MIDI channel 0 (channel 1 in most DAWs)
   - Values: 127 (pressed), 0 (released)
   
   **Note:** These CC assignments are hard-coded in the PG_BT4 hardware and cannot be changed.

### MIDI Mode

The bridge creates virtual MIDI ports named "PG_BT4 Bridge":
- **Source Port**: Receives button presses from the controller
- **Destination Port**: Sends LED control commands to the device
...
**Important:** The button CC assignments (80-83) are fixed in the PG_BT4 hardware and cannot be remapped.

### Keyboard Mode (Menu Bar App Only)

In keyboard mode, PG_BT4 buttons send keyboard events instead of MIDI:
...
2. Switch to "⌨️ Keyboard" mode
3. Click "Keyboard Settings..."
4. Click a button field and press your desired key
5. Click "Done" to save

You can map to:
- Plain keys (A-Z, Space, Return, etc.)
- Keys with modifiers (⌘A, ⇧Space, etc.)
- Function keys (F1-F12)
- Arrow keys and special keys

**Note:** Keyboard mode requires Accessibility permissions (macOS will prompt when first used).
